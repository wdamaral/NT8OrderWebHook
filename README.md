...
**OrderWebHook** is a custom NinjaScript indicator designed to bridge the gap between NinjaTrader 8 executions and external automation platforms. It monitors specific accounts for trade executions and forwards the data instantaneously via HTTP Webhooks to configured endpoints (ATS and QuantLynk) or locally via **Windows Named Pipes**.

## **🚀 Features**

* **Real-Time Monitoring**: Hooks into **OnExecutionUpdate** to capture fills immediately.
* **Multi-Provider Architecture**: Capable of sending data to multiple services simultaneously (ATS, QuantLynk, and Local Pipes).
* **Ultra-Low Latency via Named Pipes**: Sends payloads to local Windows pipes with **<100ms latency**, significantly faster than standard HTTP webhooks.
* **Bypass Automation Limits**: Uses local inter-process communication (IPC) to keep data off the web. This setup allows you to execute trades on external platforms while bypassing automation limitations found on certain prop firms.
* **Asynchronous Processing**: Uses a Producer/Consumer pattern (BlockingCollection) to ensure requests do not block the UI or Trading thread.
...
* NinjaTrader 8
* An active internet connection (for Webhooks)
* Target webhook URLs (ATS or QuantLynk endpoints)
* *(Optional)* A local application or script listening on the configured Named Pipe to receive trade data.
...
### **3. Named Pipe Settings (Local)**

* **Enable Pipes**: Master switch for local pipe transmission.
* **Pipe Name**: The name of the pipe to broadcast to (e.g., NinjaStream). Ensure your listening application is tuned to this specific pipe name.
...
1.  **Status Buttons**:
    * **Green**: The service (Webhook or Pipe) is active.
    * **Red**: The service is disabled.
    * *Clicking these buttons toggles the service instantaneously without reloading the script.*
2.  **Log Box**:
    * Displays successful HTTP status codes.
    * Displays Pipe transmission confirmation (e.g., PIPE: Sent (12ms)).
...
* **PipeWriter**: Establishes a NamedPipeClientStream to a local server. This operates on a **fire-and-forget** basis to maintain **sub-100ms speeds** and zero UI blocking.
